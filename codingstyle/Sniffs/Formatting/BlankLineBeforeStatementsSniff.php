 ...
 * @link     https://github.com/djoos/Symfony-coding-standard
 */
namespace codingstyle\Sniffs\Formatting;
 ...
 * @link     https://github.com/djoos/Symfony-coding-standard
 */
class BlankLineBeforeStatementsSniff implements Sniff
{
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $current         = $stackPtr;
        $currentToken     = $tokens[$current]['type'];
        ...
        if (isset($prevLineTokens[0])
            && ($prevLineTokens[0] === 'T_OPEN_CURLY_BRACKET'
                || $prevLineTokens[0] === 'T_COLON' || $prevLineTokens[0] === 'T_DOC_COMMENT_STRING')
        ) {
            return;
        }
        ...
        if (count($prevLineTokens) > 0) {
            $fix = $phpcsFile->addFixableError(
                'Missing blank line before ' . match($currentToken) {
                    'T_RETURN' => 'return',
                    'T_IF' => 'if',
                    'T_FOR' => 'for',
                } . ' statement',
